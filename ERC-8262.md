...
7. Reject any other `analysis_type`. Analysis type identifiers 1-127 are reserved for this specification. Identifiers 128-255 are available for deployment-defined types.
...
| --- | ------------ | --------- | ------------ | --------------------------- |
| 0   | EU (AMLD6 / AMLR 2024/1624)      | 0-3099    | 3100-7099    | >=7100                      |
...
These thresholds are illustrative reference defaults chosen by the specification authors; no regulator publishes basis-point risk thresholds. Deployments SHOULD calibrate them to their jurisdictional context.
...
| --- | --------------- | ----------------------------- | ------------------------ |
| 0   | EU (AMLD6 / AMLR 2024/1624)      | yes                           | 1                        |
| 1   | US (BSA)        | no                            | 2                        |
| 2   | UK (MLR)        | yes                           | 1                        |
...
**Verifier revocation and attestation validity.** Implementations MUST treat stored attestations whose `verifierUsed` maps to a revoked verifier version as invalid when queried via `checkCompliance()` or `checkComplianceByType()`. The `IERC8262Verifier` interface includes `isVerifierAddressRevoked(uint8 proofType, address verifierAddr)` for this lookup. This ensures that attestations minted by a verifier later found unsound do not survive the remainder of their TTL.
...
| COMPLIANCE_MULTI_SIGNED | jurisdiction_id, provider_set_hash, config_hash, meets_threshold, threshold_m, signer_pubkey_hash_0..4 (registered + distinct), chain_id == block.chainid, oracle_address == address(this), threshold_m >= minMultiProviderThreshold(jurisdictionId) | Config hash + signer-pubkey-hash registries |
...
The `providerSetHash` parameter in `submitCompliance()` is semantically meaningful for COMPLIANCE-family proofs (COMPLIANCE 0x01, COMPLIANCE_SIGNED 0x07, COMPLIANCE_MULTI_SIGNED 0x09), which include it as a caller-supplied public input. RISK_SCORE proofs also commit to a `provider_set_hash` in their circuit public inputs, but this value is embedded in the proof itself and does not come from the caller parameter. For all non-COMPLIANCE-family proof types, implementations MUST ignore the caller-supplied `providerSetHash` and store `bytes32(0)` in the attestation to prevent injection of arbitrary values.
...
A standalone non-membership proof attests absence from a specific published set at the time of tree publication. A fresh address is trivially absent from any set. Non-membership's assurance value lies in procedural diligence and composition with ATTESTATION or provider-signed proofs, not as a standalone guarantee against motivated evaders.
...
This standard addresses pre-trade compliance screening. Post-trade data-transmission obligations (FATF Recommendation 16 / Travel Rule) for VASPs are out of scope.
