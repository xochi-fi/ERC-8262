...
Implementations MUST also implement [ERC-165](https://eips.ethereum.org/EIPS/eip-165). `supportsInterface(bytes4)` MUST return `true` for `type(IERC8262Verifier).interfaceId` and for `type(IERC165).interfaceId`, and `false` for `0xffffffff`.
...
interface IERC8262Oracle {
    struct ComplianceAttestation {
        ...
        bytes32 proofHash;        // keccak256(proof, proofType, chainId, oracleAddr) -- normative (see Proof replay prevention)
...
Implementations MUST also implement [ERC-165](https://eips.ethereum.org/EIPS/eip-165). `supportsInterface(bytes4)` MUST return `true` for `type(IERC8262Oracle).interfaceId` and for `type(IERC165).interfaceId`, and `false` for `0xffffffff`.
...
Implementations targeting a jurisdiction not enumerated above SHOULD select
policy values from the published regulatory regime (preference: signed-only +
M >= 2 when the regulator has issued a provider-attestation requirement). The
policy MUST be hard-coded or governance-mutable with the same time-delay
guarantees as verifier upgrades; it MUST NOT be settable per-submission.
...
**[ERC-1922](https://eips.ethereum.org/EIPS/eip-1922).** The original zk-SNARK verifier standard (2019, stagnant). ERC-1922 defines a generic interface for on-chain ZK verification with dynamic arrays for cross-scheme compatibility. This ERC supersedes ERC-1922's approach with per-proof-type routing, UltraHonk support, and domain-specific input validation.

## Backwards Compatibility

This ERC introduces new interfaces and does not modify existing standards. It is designed to complement [ERC-5564](https://eips.ethereum.org/EIPS/eip-5564) (stealth addresses) and [ERC-6538](https://eips.ethereum.org/EIPS/eip-6538) (stealth meta-address registry) for privacy-preserving settlement, but does not depend on them.
...
Thanks to Merkle Bonsai (@Jabher) for reviewing the generated UltraHonk verifiers and identifying that the `pairing()` free function could be rewritten in inline Yul to bring all nine per-proof-type verifiers under the [EIP-170](https://eips.ethereum.org/EIPS/eip-170) 24,576-byte runtime size limit. The reference implementation incorporates the rewrite in `scripts/patch-pairing-yul.sh`, saving ~186 bytes per verifier (and ~800 gas per `verifyProof` call as a bonus) while staying byte-identical to the `bb`-generated semantics on the pairing precompile (`address(0x08)`) input layout.
...
- Strict-mode jurisdictions MUST reject the unsigned siblings entirely and accept only the signed variants. The reference implementation enforces this via `JurisdictionConfig.requireSignedSignals(uint8)`; see the Jurisdiction Policy table for the canonical per-jurisdiction values.
