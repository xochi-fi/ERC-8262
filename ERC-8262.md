...
author: DROO (@DROOdotFOO), Bloo (@bloo-berries), Merkle Bonsai (@Jabher), Jan (@sssngth)
...
All proof types include `submitter` as a public input. The checks applied to it, and to
every other public input, are specified in [Public Input Validation](#public-input-validation).
...
interface IERC8262Oracle {
    struct ComplianceAttestation {
        ...
        bytes32 providerSetHash;  // hash of providers + weights (0x01/0x07/0x09 only; bytes32(0) otherwise)
...
This section governs the *submission boundary* only. It is distinct from, and additional to, the in-circuit constraints in [Per-Type Circuit Specifications](#per-type-circuit-specifications): the circuit constrains the relationship between inputs, while the Oracle constrains their meaning in this deployment. A proof that satisfies its circuit but fails these checks MUST be rejected.

The per-type tables below are exhaustive. Every public input of every proof type appears exactly once, at the index the circuit declares it. An index whose Oracle validation is `--` is one the Oracle deliberately does not check; the reason is stated in each such row.

#### Cross-cutting rules

The following apply wherever the named input is present. Per-type tables reference them by name rather than restating them.

| Rule                     | Applies to                                     | Requirement                                                                                                                                                                          |
| ------------------------ | ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Submitter binding**    | all proof types                                | `submitter` MUST equal `msg.sender`. See [Submitter Binding](#submitter-binding).                                                                                                    |
| **Positive result**      | all proof types                                | The boolean result field MUST be exactly `bytes32(uint256(1))`. See [Proof Result Validation](#proof-result-validation).                                                             |
| **Timestamp freshness**  | 0x01, 0x04, 0x05, 0x06, 0x07, 0x09             | The timestamp is chosen by the prover, not derived from the submitting transaction, and the circuit bounds it only loosely (see [Circuit Constraints](#circuit-constraints)). It MUST NOT be in the future and MUST be within an implementation-defined maximum age (reference: 1 hour), so that a backdated or long-stale proof cannot be presented as current. Types without a timestamp input are exempt. |
| **Timestamp ratchet**    | all proof types                                | Implementations MUST maintain a non-decreasing per-`(subject, jurisdiction)` timestamp and reject submissions below it, so a stale proof cannot overwrite a newer attestation. Equal values MUST remain acceptable. Types without a timestamp input ratchet on `block.timestamp`. |
| **Jurisdiction policy**  | 0x01, 0x02 (rejection), 0x09 (`threshold_m`)   | Unsigned screening proofs MUST be rejected for jurisdictions requiring signed signals. See [Jurisdiction Policy](#jurisdiction-policy).                                              |

#### COMPLIANCE (0x01)

| #   | Public input        | Oracle validation                                                                                                             |
| --- | ------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| 0   | `jurisdiction_id`   | MUST equal the `jurisdictionId` argument of `submitCompliance()`.                                                             |
| 1   | `provider_set_hash` | MUST equal the `providerSetHash` argument of `submitCompliance()`.                                                            |
| 2   | `config_hash`       | MUST be registered and non-revoked in the config hash registry, and MUST NOT expand to any denied provider.                   |
| 3   | `timestamp`         | Timestamp freshness; timestamp ratchet.                                                                                       |
| 4   | `meets_threshold`   | Positive result.                                                                                                              |
| 5   | `submitter`         | Submitter binding.                                                                                                            |

#### RISK_SCORE (0x02)

| #   | Public input        | Oracle validation                                                                                                             |
| --- | ------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| 0   | `proof_type`        | MUST be 1 (threshold) or 2 (range); any other value MUST be rejected.                                                         |
| 1   | `direction`         | For `proof_type == 1`, MUST be 1 (GT) or 2 (LT). Ignored for range proofs.                                                    |
| 2   | `bound_lower`       | MUST NOT express a trivially-true or unsatisfiable claim (see below).                                                         |
| 3   | `bound_upper`       | For range proofs, MUST exceed `bound_lower` and MUST NOT exceed 10000.                                                        |
| 4   | `result`            | Positive result.                                                                                                              |
| 5   | `config_hash`       | MUST be registered and non-revoked in the config hash registry.                                                               |
| 6   | `provider_set_hash` | `--` Bound in-circuit to the committed providers and weights, but not cross-checked against any caller argument or registry.  |
| 7   | `submitter`         | Submitter binding.                                                                                                            |

Trivial-bound rejection: for `direction == GT`, `bound_lower == 0` or `bound_lower >= 10000`; for `direction == LT`, `bound_lower == 0` or `bound_lower > 10000`; for range proofs, the full domain `[0, 10000]`.

#### PATTERN (0x03)

| #   | Public input          | Oracle validation                                                                                                           |
| --- | --------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| 0   | `analysis_type`       | MUST be 1 (anti-structuring), 2 (velocity), or 3 (round-amount). Consumers requiring a specific analysis MUST check this value themselves. |
| 1   | `result`              | Positive result.                                                                                                            |
| 2   | `reporting_threshold` | MUST be registered in the reporting threshold registry.                                                                     |
| 3   | `time_window`         | MUST meet an implementation-defined minimum (reference: 3600 seconds).                                                      |
| 4   | `tx_set_hash`         | MUST be non-zero.                                                                                                           |
| 5   | `submitter`           | Submitter binding.                                                                                                          |
| 6   | `settlement_root`     | `--` Opaque by design. Consumers binding a pattern proof to downstream state MUST recompute the expected value and assert equality themselves. |

#### ATTESTATION (0x04)

| #   | Public input        | Oracle validation                                                                                                             |
| --- | ------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| 0   | `provider_id`       | MUST be non-zero and MUST equal the `providerId` recorded against the referenced credential root.                             |
| 1   | `credential_type`   | `--` Constrained in-circuit; the Oracle applies no additional check.                                                          |
| 2   | `is_valid`          | Positive result.                                                                                                              |
| 3   | `credential_root`   | MUST be registered in the credential root registry, MUST NOT be revoked, and MUST be within its TTL (reference: 48 hours).    |
| 4   | `current_timestamp` | Timestamp freshness; timestamp ratchet.                                                                                       |
| 5   | `submitter`         | Submitter binding.                                                                                                            |
...
| #   | Public input  | Oracle validation                                                                                    |
| --- | ------------- | ------------------------------------------------------------------------------------------------------ |
| 0   | `merkle_root` | MUST be registered and non-revoked in the merkle root registry.                                       |
| 1   | `set_id`      | `--` Bound in-circuit to the leaf hash; the Oracle applies no additional check.                       |
| 2   | `timestamp`   | Timestamp freshness; timestamp ratchet.                                                               |
| 3   | `is_member`   | Positive result.                                                                                      |
| 4   | `submitter`   | Submitter binding.                                                                                    |
...
| 1   | `set_id`         | `--` Bound in-circuit to the leaf hash; the Oracle applies no additional check.                    |
| 2   | `timestamp`      | Timestamp freshness; timestamp ratchet.                                                            |
| 3   | `is_non_member`  | Positive result.                                                                                   |
| 4   | `submitter`      | Submitter binding.                                                                                 |
...
| 3   | `timestamp`           | Timestamp freshness; timestamp ratchet.                                                                       |
| 4   | `meets_threshold`     | Positive result.                                                                                              |
| 5   | `signer_pubkey_hash`  | MUST be registered in the signer pubkey hash registry.                                                        |
| 6   | `chain_id`            | MUST equal `block.chainid`.                                                                                   |
| 7   | `oracle_address`      | MUST equal `address(this)`.                                                                                   |
| 8   | `submitter`           | Submitter binding.                                                                                            |

#### RISK_SCORE_SIGNED (0x08)

| #   | Public input          | Oracle validation                                                                                            |
| --- | --------------------- | -------------------------------------------------------------------------------------------------------------- |
| 0   | `proof_type`          | As RISK_SCORE index 0.                                                                                        |
| 1   | `direction`           | As RISK_SCORE index 1.                                                                                        |
| 2   | `bound_lower`         | As RISK_SCORE index 2.                                                                                        |
| 3   | `bound_upper`         | As RISK_SCORE index 3.                                                                                        |
| 4   | `result`              | Positive result.                                                                                              |
| 5   | `config_hash`         | MUST be registered and non-revoked in the config hash registry.                                               |
| 6   | `provider_set_hash`   | `--` As RISK_SCORE index 6: bound in-circuit, not cross-checked at the submission boundary.                   |
| 7   | `signer_pubkey_hash`  | MUST be registered in the signer pubkey hash registry.                                                        |
| 8   | `chain_id`            | MUST equal `block.chainid`.                                                                                   |
| 9   | `oracle_address`      | MUST equal `address(this)`.                                                                                   |
| 10  | `submitter`           | Submitter binding.                                                                                            |
...
| 3   | `timestamp`            | Timestamp freshness; timestamp ratchet.                                                                              |
| 4   | `meets_threshold`      | Positive result.                                                                                                     |
| 5   | `threshold_m`          | MUST be in `[1, MAX_PROVIDERS_MULTI]`, MUST NOT exceed the number of active signer slots, and MUST be at least the jurisdiction's `minMultiProviderThreshold`. |
| 6   | `signer_pubkey_hash_0` | If non-zero, MUST be registered in the signer pubkey hash registry and MUST be distinct from every other slot.       |
| 7   | `signer_pubkey_hash_1` | As index 6.                                                                                                          |
| 8   | `signer_pubkey_hash_2` | As index 6.                                                                                                          |
| 9   | `signer_pubkey_hash_3` | As index 6.                                                                                                          |
| 10  | `signer_pubkey_hash_4` | As index 6.                                                                                                          |
| 11  | `chain_id`             | MUST equal `block.chainid`.                                                                                          |
| 12  | `oracle_address`       | MUST equal `address(this)`.                                                                                          |
| 13  | `submitter`            | Submitter binding.                                                                                                   |
...
The `providerSetHash` parameter in `submitCompliance()` is semantically meaningful only for the compliance-family proof types (COMPLIANCE, COMPLIANCE_SIGNED, COMPLIANCE_MULTI_SIGNED). For those three, implementations MUST assert that the caller-supplied parameter equals the `provider_set_hash` public input of the proof, and MUST store it in the attestation. RISK_SCORE and RISK_SCORE_SIGNED also commit to a `provider_set_hash` in their circuit public inputs, but that value is bound inside the proof and is not cross-checked against the caller parameter; the remaining proof types have no such input at all. For every proof type outside the compliance family, implementations MUST ignore the caller-supplied `providerSetHash` and store `bytes32(0)` in the attestation to prevent injection of arbitrary values.
...
**Signer pubkey hash registry.** Tracks the `H_signer_pubkey` commitments of providers authorized to sign screening payloads, for the signed variants (0x07, 0x08, 0x09). A `signer_pubkey_hash` public input that is not registered MUST be rejected. Because the in-circuit ECDSA check proves only that *some* key signed the payload, this registry is what makes a signed proof mean "signed by an authorized provider"; without it the signed variants offer no more provenance than their unsigned siblings.

**Denied provider registry.** Tracks provider IDs that MUST NOT contribute to an accepted compliance proof, independent of whether the configuration committing to them is still registered. Implementations that support it MUST record, for each registered config hash, the provider IDs that hash expands to, and MUST reject any compliance-family proof (0x01, 0x07, 0x09) whose `config_hash` expands to a denied provider. This gives operators a response to a single compromised or delisted provider that is faster than rotating and re-registering every affected configuration. The expansion MUST be recorded atomically with the configuration registration, so a partially-applied rotation cannot silently disable enforcement.
