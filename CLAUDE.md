...
ERC-8262: a standard for zero-knowledge compliance proofs on Ethereum. Users prove AML/sanctions compliance without revealing transaction data. Regulators verify a ZK proof, never see the trade.
...
- **One verifier per proof type**: ERC8262Verifier routes to the correct generated verifier
...
- `src/interfaces/`: IERC8262Verifier, IERC8262Oracle, IUltraVerifier, ISettlementRegistry
...
- `src/`: ERC8262Verifier (routes to per-type verifiers), ERC8262Oracle (attestation storage + input validation + 8 registries + ratchet + replay), SettlementRegistry (immutable, links split settlement proofs to tradeId, references Oracle via getHistoricalProof), Timelock (2-tier 24h HIGH / 6h LOW selector-gated admin delay)
...
- Verification path is fully `view` (IUltraVerifier, IERC8262Verifier, generated verifiers)
- Proof hashes keyed on `keccak256(abi.encodePacked(proof, proofType, block.chainid, address(this)))` to prevent cross-type, cross-chain, and cross-Oracle replay (audit F-6). Use `ERC8262Oracle.computeProofHash(proof, proofType)` as the source of truth from off-chain code.
...
Per-jurisdiction policy: `JurisdictionConfig.requireSignedSignals(uint8)` returns true for US (BSA), Singapore (MAS), and UAE (VARA). The Oracle's submission dispatcher rejects unsigned COMPLIANCE / RISK_SCORE for those jurisdictions with `SignedSignalsRequired`.
